...
interface BillSplitterProps {
  receiptSnapshot: CartItem[];
  receiptTotalSnapshot: number;
  onBack: () => void;
  onDone: () => void;
  gstFee: number;
  deliveryFee: number;
...
export default function BillSplitter({
  ...
  const handleDeleteFriend = (id: number, e: MouseEvent<HTMLSpanElement>) => {
    e.stopPropagation();
  ...
  const handleAdjustPortion = (itemId: string, change: number) => {
    setFriendsList(
      friendsList.map((f) => {
  ...
  const getPortionsClaimedCount = (itemId: string) => {
  ...
  const calculateSingleFriendBill = (friend: Friend) => {
    const friendFoodSubtotal = friend.items.reduce((sum, assignment) => {
      const targetItem = receiptSnapshot.find(
        (i) => String(i.id) === String(assignment.itemId),
      );
    ...
    const totalRawFoodCost = receiptSnapshot.reduce(
  ...
  const handleLaunchWhatsAppWebSync = () => {
    ...
    friendsList.forEach((f) => {
      ...
      const itemsBulletListText = f.items
        .map((assignment) => {
          const dish = receiptSnapshot.find(
            (i) => String(i.id) === String(assignment.itemId),
          );
    ...
    window.open(
      `${whatsappBaseUrl}/91${cleanNumber}?text=${cleanPayload}`,
      "_blank",
    );
  };
          ...
          return (
            <div
              ...
              className={`${styles.itemCardBase} ${
                claimedQty > 0
                  ? styles.itemCardClaimed
                  : styles.itemCardUnclaimed
              }`}
            >
              ...
              <div className={styles.stepperContainer}>
                ...
                <span className={styles.stepperValueDisplay}>{claimedQty}</span>
                <button
                  onClick={() => handleAdjustPortion(item.id, 1)}
...
const styles = {
  container:
    "flex flex-col h-full min-h-0 justify-between w-full relative text-zinc-100 overflow-y-auto",

  headerSection:
    "flex flex-col gap-1.5 pb-2.5 border-b border-zinc-800 shrink-0 w-full",
  headerLabel:
    "text-[10px] uppercase font-black text-orange-400 tracking-wider",

  inputRow: "flex flex-col sm:flex-row gap-2 w-full mt-0.5",

  friendInput:
    "bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 text-xs w-full sm:flex-1 text-zinc-200 outline-none focus:border-zinc-700 font-sans shadow-inner placeholder:text-zinc-700 shrink-0",

  addButton:
    "w-full sm:w-auto bg-zinc-800 hover:bg-zinc-700 text-zinc-200 px-4 py-1.5 rounded-xl text-xs font-black tracking-wide uppercase transition duration-150 cursor-pointer text-center flex items-center justify-center shrink-0 shadow-md shadow-black/10",

  friendsScrollContainer:
    "flex flex-wrap  gap-1.5 overflow-y-visible  py-2.5 border-b border-zinc-800/40 shrink-0 max-w-full items-center px-1 w-full min-h-0",
  friendTabBase:
    "px-2.5 py-1 rounded-xl text-[10px] font-bold transition cursor-pointer shrink-0 flex items-center gap-1.5 border group select-none min-h-6",

  friendTabActive: "bg-orange-500/10 border-orange-500 text-orange-400",
  friendTabInactive:
    "bg-zinc-950/40 border-zinc-800/60 text-zinc-500 hover:text-zinc-400",
  deleteFriendIcon:
    "text-[9px] text-zinc-600 hover:text-red-400 transition font-mono px-0.5",

  allocationList:
    "flex-1 min-h-70 overflow-y-auto custom-scrollbar flex flex-col gap-2 py-3 pr-1",
  allocationInstruction:
    "text-[9px] uppercase tracking-wider text-zinc-500 font-bold mb-1",
  ...
  itemCardUnclaimed:
    "bg-zinc-950/10 border-zinc-800/30 text-zinc-500 opacity-70",
  ...
  stepperContainer:
    "flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-lg p-1 shrink-0",
  stepperBtnMinus:
    "w-5 h-5 rounded bg-zinc-800 text-zinc-400 hover:text-white font-bold transition flex items-center justify-center text-xs cursor-pointer select-none",
  stepperValueDisplay:
    "font-mono font-bold text-xs text-zinc-200 w-4 text-center select-none",
  stepperBtnPlusBase:
    "w-5 h-5 rounded font-bold transition flex items-center justify-center text-xs select-none",
  stepperBtnPlusAvailable:
    "bg-zinc-800 text-zinc-400 hover:text-white cursor-pointer",
  stepperBtnPlusDisabled: "bg-zinc-950/20 text-zinc-700 cursor-not-allowed",

  footerPanel:
    "shrink-0 pt-3 border-t border-zinc-800 flex flex-col gap-2.5 mt-auto w-full",
  ...
  paymentInputBase:
    "bg-zinc-950 border rounded-xl px-3 py-1.5 text-xs text-zinc-200 focus:outline-none w-full placeholder:text-zinc-700 font-mono transition duration-150",
  paymentInputValid: "border-zinc-800 focus:border-zinc-700",
  paymentInputError: "border-red-500/80 focus:border-red-500",

  validationErrorMessage:
    "text-[9px] text-red-400 font-medium px-1 mt-0.5 animate-in fade-in duration-100",
  ...
  whatsappShareBtn:
    "w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white rounded-xl text-xs font-bold transition text-center cursor-pointer shadow-lg shadow-emerald-950/20 flex items-center justify-center gap-1.5",

  backNavBtn:
    "text-[10px] text-zinc-500 hover:text-zinc-300 font-bold uppercase tracking-wider transition text-center cursor-pointer py-0.5",
};
